#!/bin/bash
#$ -N P2 #Job ID name sent to HPC cluster.
#$ -pe openmp 5 #Number of requested parallel environments.
#$ -o '/<directory root>/fMRIPrep/Pipeline_2/logs' #Folder where fMRIPrep logs files are saved out by the HPC cluster.
...
SCRATCH_DIR=/<directory root>/Pipeline_2/scratch
OUT_DIR=/<directory root>/Pipeline_2/derivatives
LICENSE=/<directory root>/fs_license/license.txt
LOCAL_FREESURFER_DIR=/<directory root>/Pipeline_2/derivatives/sourcedata/freesurfer
...
singularity run --cleanenv \
    ...
    --sloppy \ #P2: Turns on sloppy testing mode.
