#!/bin/bash
#$ -N P1 #Job ID name sent to HPC cluster.
#$ -pe openmp 5 #Number of requested parallel environments.
#$ -o '/<directory root>/fMRIPrep/Pipeline_1/logs' #Folder where fMRIPrep logs files are saved out by the HPC cluster.
...
SCRATCH_DIR=/<directory root>/Pipeline_1/scratch
OUT_DIR=/<directory root>/Pipeline_1/derivatives
LICENSE=/<directory root>/fs_license/license.txt
LOCAL_FREESURFER_DIR=/<directory root>/Pipeline_1/derivatives/sourcedata/freesurfer
...
singularity run --cleanenv \
    ...
    --fs-no-reconall \ #P1: FreeSurfer surface reconstruction switched off.
